...
string CPatternScanner::Basename( string const& pathname )
{
...
uintptr_t CPatternScanner::GetPointer(string imageName, unsigned char* bMask, const char* szMask, uint32_t start)
{
...
uintptr_t CPatternScanner::GetProcedure(string imageName, unsigned char* bMask, const char* szMask, uintptr_t offset)
{
    MemoryModule module = loaded_modules[imageName];
    uintptr_t dwAddress = module.address;
    off_t dwLen = module.length;
...
string CPatternScanner::GetModulePath(string imageName)
{
    return loaded_modules[imageName].path;
}

uintptr_t CPatternScanner::GetBaseAddress(string imageName)
{
    MemoryModule module = loaded_modules[imageName];
